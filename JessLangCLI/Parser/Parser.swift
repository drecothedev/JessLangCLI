...
final class Parser {
    ...
    private func classDeclaration() throws -> Stmt  {
        ...
        _ = try consume(.leftBrace, message: "Expect left brace")
        ...
        _ = try consume(.rightBrace, message: "Expect '}' after class body.")
        
        return Stmt.class(name: name, methods)
    ...
    private func call() throws -> Expr {
        ...
        while true {
            if match(.leftParent) {
                ...
                let name: Token = try consume(.identifier, message: "Expect name after '.'.")
                expr = Expr.get(expr: expr, name: name)
            } else {
                break
            }
        }
