...
final class Interpreter: ExprVisitor {
    ...
    func visitGetExpr(expr: Expr, name: Token) throws -> Any? {
        ...
        if let obj = obj as? JessInstance {
            return try obj.get(name)
        }
        
        throw RuntimeError(token: Token().defaultToken(), message: "Only instances have properties.")
    }
    ...
    private func execute(_ stmt: Stmt) throws {
        ...
        case .class(let name, let methods):
            try environment.define(name: name, value: methods)
            let klass: JessClass = JessClass(name: name.lexeme)
            try environment.assign(name: name, value: klass)
        }
    }
